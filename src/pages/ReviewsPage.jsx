...
import { Star, Send, CheckCircle2, Instagram, MapPin, ExternalLink } from 'lucide-react';
        ...
        <div className="bg-bg-light dark:bg-bg-dark min-h-screen pt-32 pb-24 px-6 relative overflow-hidden">
            ...
            <div className="max-w-2xl mx-auto relative z-10">
                ...
                <motion.div
                    ...
                    className="bg-white dark:bg-white/5 border border-black/5 dark:border-white/5 p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-black/5 backdrop-blur-sm"
                >
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        {/* Auth Checkbox - PRIDE / SOCIAL OPT-IN */}
                        <div
                            className={`p-6 rounded-[2rem] border-2 transition-all cursor-pointer group flex items-start gap-4 ${formData.auth ? 'bg-primary/5 border-primary shadow-sm' : 'bg-white dark:bg-white/5 border-black/5 dark:border-white/5 hover:border-primary/30'}`}
                            onClick={() => setFormData({ ...formData, auth: !formData.auth })}
                        >
                            <div className={`mt-1 min-w-[24px] h-6 rounded-lg border-2 transition-colors flex items-center justify-center ${formData.auth ? 'bg-primary border-primary' : 'border-black/10 dark:border-white/10'}`}>
                                {formData.auth && <CheckCircle2 size={14} className="text-white" />}
                            </div>
                            <div className="flex-1">
                                <p className={`text-sm font-bold mb-1 transition-colors ${formData.auth ? 'text-primary' : 'text-gray-700 dark:text-gray-300'}`}>
                        ...
                        {status === 'error' && (
                            <motion.div
                                ...
                                className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 p-4 rounded-xl text-red-600 dark:text-red-400 text-sm font-bold text-center"
                            >
                                {t('common.error')}. Por favor, inténtalo de nuevo o contacta con nosotros.
                            </motion.div>
                        )}

                        {/* Submit Button & Implicit Consent */}
                        <div className="space-y-4">
                            <button
                                type="submit"
                                disabled={status === 'loading'}
                                className={`w-full py-5 rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-3 ${status === 'loading' ? 'bg-gray-100 dark:bg-white/5 text-gray-400 cursor-not-allowed' : 'btn-primary shadow-xl shadow-primary/20'}`}
                            >
                                {status === 'loading' ? (
